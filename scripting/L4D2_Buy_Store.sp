...
{
	{"health_100", 		"Health+100", 		"500"},
	{"defibrillator",	"Defibrillator", 	"500"},
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_zs", SuicideCommand);
	RegConsoleCmd("sm_suicide", SuicideCommand);
	RegConsoleCmd("sm_kill", SuicideCommand);
...
public Action SuicideCommand(int client, int args)
{
	if(GetClientTeam(client) != L4D_TEAM_SURVIVORS)
	{
		ReplyToCommand(client, "[TS] Survivors only.");
		return Plugin_Handled;
	}

	if(!IsClientInGame(client) || !IsAliveSurvivor(client))
	{
		ReplyToCommand(client, "[TS] You were dead.");
		return Plugin_Handled;
	}
