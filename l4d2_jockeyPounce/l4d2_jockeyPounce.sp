...
{
    g_hEnabled = CreateConVar( "l4d2_JockeyPounce_enabled", "1", "Should the plugin be enabled" );
    g_hBlindAmount = CreateConVar( "l4d2_JockeyPounce_blind", "150", "How much the jockey should blind the player (0: non, 255:completely)" );
    g_hPounceScale = CreateConVar( "l4d2_JockeyPounce_scale", "1.0", "Scale how much damage the pounce does (e.g. 0.5 will half the default damage, 5 will make it 5 times more powerfull)" );
    g_hPounceCap = CreateConVar( "l4d2_JockeyPounce_cap", "25", "Cap of the maximum damage a pounce can do" );
    g_hPounceMinShow = CreateConVar( "l4d2_JockeyPounce_minshow", "1", "Minimum damage a pounce should do to show the pounce message" );
    g_hPounceDisplay = CreateConVar( "l4d2_JockeyPounce_display", "0", "How message should be shown, 0 - Disabled, 1 - Chat message, 2 - Hint Message" );
    g_hPounceDisplayMax = CreateConVar( "l4d2_JockeyPounce_display_max", "0", "Show the damagecap in the display message" );
...
stock DistanceJumped( client, victim )
{
	...
	if( damage >= GetConVarInt(g_hPounceMinShow) )
	{
		...
		{
			if( GetConVarInt( g_hPounceDisplay ) == 1 ) CPrintToChatAll("{default}[{blue}!{default}] {olive}%N {default} pounced {olive}%N {default}for {blue}%d {default} damage", client, victim, damage);	
			if( GetConVarInt( g_hPounceDisplay ) == 2 ) PrintHintTextToAll( "%N pounced %N for %d damage", client, victim, damage);
		}
	}
